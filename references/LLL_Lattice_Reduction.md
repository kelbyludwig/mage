...
## 1. Two Dimensional Reduction and Gram-Schmidt
...
Instead of *just* taking the projection of each vector onto subsequent vectors, we
...
Is the vector result of `proj_round` a lattice vector?
[Yarp.](http://img.pandawhale.com/126825-hot-fuzz-YARP-meme-Imgur-rory-U4l3.jpeg)
...
Lets say we took Gram-Schmidt, and replaced `proj` with `proj_round`. Let's
...
## 2. LLL and GS - Friends Forever

Tampering with the internals of Gram-Schmidt didn't get us what we want.
However, Gram-Schmidt will still be very useful as we begin our path to LLL. In
...
(i.e. a series of basis vectors that are "better" than the input, yet span the
same space), but both leverage vector projections as a way to remove redundancy
...
* What is mu(i,j) measuring?
    ...
    * mu(i,j) provides a measurement of the angle formed by `B[i]` and the `Q[j]` 
        ...
        * TODO(kkl) differences in length can also affect mu since we are dealing with projections
